...
# Attention Ablation Variants

#Simple concatenation, no attention
def build_no_attention(backbone="cnn4", mode="joint"):
    ...
    palm_pool = layers.GlobalAveragePooling2D()(palm_feat)  
    nail_pool = layers.GlobalAveragePooling2D()(nail_feat)  

   
    meta = layers.Dense(32,  activation="relu",kernel_regularizer=regularizers.l2(1e-4))(meta_input)
    meta = layers.Dense(128, activation="relu",kernel_regularizer=regularizers.l2(1e-4))(meta)


    fused = layers.Concatenate()([palm_pool, nail_pool, meta])  # Concatenate without attention
...
# Self attention only, no cross or fusion

def build_self_only(backbone="cnn4", mode="joint"):
    ...
    palm_pool = layers.GlobalAveragePooling1D()(palm_self)  
    nail_pool = layers.GlobalAveragePooling1D()(nail_self) 
...
# Self + Cross attention, no fusion

def build_self_cross(backbone="cnn4", mode="joint"):
...
def build_fusion_only(backbone="cnn1", mode="joint"):
    ...
    fused = attention_fusion(layers.Reshape((1, 128))(palm_pool),layers.Reshape((1, 128))(nail_pool), meta_out)
